#!/usr/bin/env python3
"""
Testbed experiment coordinator (dynamic mode), modeled after
simulation_bringup/experiment_coordinator.py.

Key behavior:
  - Step target -> wait for /experiment/step_stamp token
  - Trigger predictor. While predictor is bootstrapping (success=False), keep stepping.
  - Once predictor is READY, run planning loop:
      generate candidates -> capture -> voxelize -> score -> move pursuer to best

Testbed adaptations:
  - No Gazebo spawner services.
  - Optional "dynamic seed" uses closest candidate (to current pursuer pose) instead of random spawn.
"""
...
class TestbedExperimentCoordinator(Node):
    def __init__(self):
        super().__init__("experiment_coordinator_testbed")
        ...
        # Small settle time before capture so depth/TF streams stabilize
        ...
        # TF for pursuer pose (for closest-candidate seeding)
        self.declare_parameter("world_frame", "world")
        self.declare_parameter("pursuer_base_frame", "pursuer/base_link")
        self.declare_parameter("tf_lookup_timeout_sec", 0.2)

        # Dynamic seed behavior (align with simulation seed, but choose closest candidate)
        self.declare_parameter("seed_closest_candidate_dynamic", True)
        self.declare_parameter("seed_wait_after_move_sec", 1.0)

        # Fallback protocol:
        # If scoring cannot compute ellipsoids (common when frontier is degenerate),
        # optionally fall back to a deterministic candidate rather than aborting the run.
        self.declare_parameter("fallback_on_no_ellipsoids_fit", True)
        self.declare_parameter("fallback_candidate_from_end", 1)  # 1 => last, 2 => second last, ...
        ...
        self.seed_closest = bool(self.get_parameter("seed_closest_candidate_dynamic").value)
        self.seed_wait_after_move_sec = float(self.get_parameter("seed_wait_after_move_sec").value)
        self.fallback_on_no_ellipsoids_fit = bool(self.get_parameter("fallback_on_no_ellipsoids_fit").value)
        self.fallback_candidate_from_end = int(self.get_parameter("fallback_candidate_from_end").value)
        ...
        # Latching run_id publisher (so downstream can group outputs)
        ...
        self.get_logger().info(f"🧭 mode=testbed_dynamic(iter={self.iteration})")
        self.get_logger().info(f"🟣 token: {self.step_stamp_topic}")
        self.get_logger().info(f"🔧 step: {self.step_srv}  predict: {self.predict_srv}")
        self.get_logger().info(f"🔧 gen: {self.generate_srv}  score: {self.score_srv}")
        self.get_logger().info(f"🔧 capture: {self.capture_srv}  voxelize: {self.voxelize_srv}")
        self.get_logger().info(f"🔧 move: {self.pursuer_srv}")
        self.get_logger().info(f"🧪 seed_closest_candidate_dynamic={self.seed_closest}")
        self.get_logger().info("----------------------------------------------------------------")
    ...
    def _seed_move_closest_candidate(self) -> bool:
        with self._cand_mtx:
        ...
        # Publish the corresponding OPTICAL candidate as /nbv/best_candidate so pursuer_mover can convert it.
        msg = PoseStamped()
        ...
        self.get_logger().info(f"🧪 seed: closest candidate i={best_i} d={math.sqrt(best_d2):.3f}m -> publish {self.best_candidate_topic}")

        mv = call_trigger(self, self.cli_pursuer, "pursuer(seed_closest)", self.service_timeout, require_success=False)
        self.get_logger().info(f"🧪 seed move: success={bool(mv.success)} msg='{mv.message}'")
        return bool(mv.success)

    def _fallback_publish_last_candidate(self) -> bool:
        """
        Publish a deterministic fallback /nbv/best_candidate from the most recent candidate set.

        Note: we publish the OPTICAL candidate pose (world->optical) so pursuer_mover can convert
        it to a base goal (matching the normal pipeline).
        """
        with self._cand_mtx:
            opt = self._cand_opt
            base = self._cand_base
        if opt is None or base is None:
            return False
        if len(opt.poses) == 0 or len(base.poses) == 0:
            return False
        if len(opt.poses) != len(base.poses):
            return False

        k = int(self.fallback_candidate_from_end)
        if k <= 0:
            k = 1
        idx = len(opt.poses) - k
        idx = max(0, min(idx, len(opt.poses) - 1))
        ...
        self.get_logger().warn(
            f"🟠 FALLBACK: publish /nbv/best_candidate = candidate[{idx}] (from_end={k}) frame={msg.header.frame_id}"
        )
        return True
    ...
    def run(self):
        ...
        while rclpy_ok():
            ...
            if not pred.success:
                ...
                # Bootstrap stage (aligned with simulation): step again to accumulate measurements.
                if "Bootstrapping" in msg:
                    bootstrapping = True
                    self.get_logger().info("🔄 BOOTSTRAP: predictor not ready; stepping again.")
                    continue
                # If we already processed this token, that's OK: proceed to generator retries.
                if "already processed" in msg:
                    pass
                else:
                    # Other predictor failures: step again.
            ...
            while rclpy_ok() and time.time() < gen_deadline:
                ...
                if "Prediction stamp is older than step token" in str(gen.message):
                    ...
                    continue
                # Some other failure: give up this token and step again.
            ...
            # Wait for the candidates topics to arrive for this generation.
            self._wait_for_candidates(cand_seq0, timeout_sec=max(0.5, self.token_timeout))

            # Optional dynamic seed: move to closest candidate ONCE (instead of random spawn).
            if (not bootstrapping) and self.seed_closest and (not did_seed):
                try:
                    _ = self._seed_move_closest_candidate()
                except Exception as e:
                    self.get_logger().warn(f"⚠️ seed failed: {e}")
                did_seed = True
                if self.seed_wait_after_move_sec > 0.0:
                    time.sleep(float(self.seed_wait_after_move_sec))
                # After seeding, restart loop to get a fresh token and predictor update.
                self.get_logger().info("🧪 seed complete; restarting loop for next token.")
                continue
            ...
            # 6) voxelize
            vox = call_trigger(self, self.cli_voxelize, "voxelizer", self.service_timeout, require_success=True)
            self.get_logger().info(f"🔧 voxelizer: success={bool(vox.success)} msg='{vox.message}'")

            # 7) score (publishes /nbv/best_candidate)
            sc = call_trigger(self, self.cli_score, "scorer", self.service_timeout, require_success=False)
            self.get_logger().info(f"🔧 scorer: success={bool(sc.success)} msg='{sc.message}'")
            if not bool(sc.success):
                msg = str(sc.message)
                if self.fallback_on_no_ellipsoids_fit and ("no ellipsoids fit" in msg.lower()):
                    ok = self._fallback_publish_last_candidate()
                    if not ok:
                        raise RuntimeError(f"scorer failed: {msg} (and fallback candidate publish failed)")
                else:
                    raise RuntimeError(f"scorer failed: {msg}")

            # 8) move pursuer to best
...
def main():
    rclpy.init()
    node = TestbedExperimentCoordinator()
    try:
        node.run()
